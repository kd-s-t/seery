import { getCoinImageUrl as getCoinImageUrlFromAPI } from '@/lib/seery/images'
...
export function getCryptoImageUrl(cryptoId: string, size: 'small' | 'large' | 'thumb' = 'small'): string {
  ...
  return `${API_URL}/api/crypto/image?cryptoId=${encodeURIComponent(id)}&size=${size}`
}

export async function getCryptoImageUrlAsync(cryptoId: string, size: 'small' | 'large' | 'thumb' = 'small'): Promise<string> {
  if (!cryptoId) {
    return ''
  }
  
  try {
    return await getCoinImageUrlFromAPI(cryptoId, size)
  } catch (error) {
    const id = cryptoId.toLowerCase().trim()
